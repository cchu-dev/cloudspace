...
export function migrateDatabase(sqlite: Database.Database): void {
  const migrate = sqlite.transaction(() => {
    sqlite.exec(`
      create table if not exists cloudspace_schema_migrations (
    ...
    const applied = new Set(
      (
        sqlite.prepare("select version from cloudspace_schema_migrations").all() as Array<{
          version: number;
        }>
      ).map((row) => row.version),
    );
    const recordMigration = sqlite.prepare(
      "insert into cloudspace_schema_migrations (version, name, applied_at) values (?, ?, ?)",
    );
