...
const REVIEW_REF_PREFIX = "refs/cloudspace/review";
...
async function createWorkingTreeSnapshot(gitRoot: string): Promise<string> {
  const tempDir = await mkdtemp(join(tmpdir(), "cloudspace-review-index-"));
  ...
  try {
    ...
    return (await git(gitRoot, ["commit-tree", tree, "-p", parent, "-m", "Cloudspace review snapshot"], { env })).stdout.trim();
  } finally {
...
function checkpointEnv(indexPath: string): NodeJS.ProcessEnv {
  return {
    ...
    GIT_AUTHOR_NAME: "Cloudspace",
    GIT_AUTHOR_EMAIL: "cloudspace@users.noreply.local",
    GIT_COMMITTER_NAME: "Cloudspace",
    GIT_COMMITTER_EMAIL: "cloudspace@users.noreply.local",
  };
}
