...
const root = await mkdtemp(join(tmpdir(), "cloudspace-skills-test-"));
...
try {
  ...
  const disabledConfig = loadConfig({
    CLOUDSPACE_ALLOWED_ROOTS: projectRoot,
    CLOUDSPACE_AGENT_DIR: agentDir,
    CLOUDSPACE_SKILL_PATHS: explicitSkills,
    CLOUDSPACE_SKILLS: "0",
  ...
  const config = loadConfig({
    CLOUDSPACE_ALLOWED_ROOTS: projectRoot,
    CLOUDSPACE_AGENT_DIR: agentDir,
    CLOUDSPACE_SKILL_PATHS: [explicitSkills, "~/.claude/skills", "./.claude/skills"].join(","),
    CLOUDSPACE_OAUTH_OWNER_TOKEN: "test-owner-token-that-is-long-enough",
    PORT: "1",
  });
  ...
  const duplicateConfig = loadConfig({
    CLOUDSPACE_ALLOWED_ROOTS: projectRoot,
    CLOUDSPACE_AGENT_DIR: agentDir,
    CLOUDSPACE_SKILL_PATHS: [explicitSkills, "./.agents/skills"].join(","),
    CLOUDSPACE_OAUTH_OWNER_TOKEN: "test-owner-token-that-is-long-enough",
    PORT: "1",
  });
  assert.equal(
  ...
  const legacyPiConfig = loadConfig({
    CLOUDSPACE_ALLOWED_ROOTS: projectRoot,
    CLOUDSPACE_AGENT_DIR: agentDir,
    CLOUDSPACE_SKILL_PATHS: [explicitSkills, join(projectRoot, ".pi", "skills")].join(","),
    CLOUDSPACE_OAUTH_OWNER_TOKEN: "test-owner-token-that-is-long-enough",
    PORT: "1",
  });
  assert.equal(
