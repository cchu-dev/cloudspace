...
Cloudspace exposes local coding capabilities over MCP. Treat it as remote access
...
Cloudspace only opens workspaces under configured roots.
...
`cloudspace init` generates an Owner password and stores it in:

```text
~/.cloudspace/auth.json
```

When an MCP client connects, Cloudspace shows an approval page. Enter the Owner
...
Cloudspace needs `CLOUDSPACE_PUBLIC_BASE_URL` so MCP clients can discover OAuth
...
Do not include `/mcp` in `CLOUDSPACE_PUBLIC_BASE_URL`.

By default, Cloudspace derives allowed Host headers from the local host and public
URL. Use `CLOUDSPACE_ALLOWED_HOSTS=*` only for intentional local debugging.

## Tunnels

Cloudspace does not manage tunnels. Your tunnel or reverse proxy should point to:
...
protection in front of public tunnels. Cloudspace OAuth still protects the MCP
...
Filesystem path containment applies to Cloudspace file tools. Shell commands run
...
By default, Cloudspace logs requests and tool calls. Shell command previews are
disabled unless `CLOUDSPACE_LOG_SHELL_COMMANDS=1`.
